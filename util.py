...
class Util:
    ...
    def split_data(self, df):
        ...
        X_test_scaled = scaler.transform(X_test)  # transform validation data
    ...
    def input_data_fields(self, overwrite_vals=None):
        ...
        concave_points_worst = col1.number_input("Concave Points Worst", format="%.2f", value=default_vals['concave points_worst'])
        perimeter_worst = col2.number_input("Perimeter Worst", format="%.2f", value=default_vals['perimeter_worst'])
        concave_points_mean = col1.number_input("Concave Points Mean", format="%.2f", value=default_vals['concave points_mean'])
        radius_worst = col2.number_input("Radius Worst", format="%.2f", value=default_vals['radius_worst'])
        perimeter_mean = col1.number_input("Perimeter Mean", format="%.2f", value=default_vals['perimeter_mean'])
        area_worst = col2.number_input("Area Worst", format="%.2f", value=default_vals['area_worst'])
        radius_mean = col1.number_input("Radius Mean", format="%.2f", value=default_vals['radius_mean'])
        area_mean = col2.number_input("Area Mean", format="%.2f", value=default_vals['area_mean'])
        concavity_mean = col1.number_input("Concavity Mean", format="%.2f", value=default_vals['concavity_mean'])
        concavity_worst = col2.number_input("Concavity Worst", format="%.2f", value=default_vals['concavity_worst'])
    ...
    def sample_data(self, df):
        test_data = df[self.features].to_dict(orient='records')
        return test_data
    ...
    def page_footer(self):
        ...
        <div class="footer">
            <p>Developed by <a style='display: block; text-align: center;' target="_blank">Gitesh Kambli, Amit Maity, Chirag Maniyath, Rishi More for Honors in AIML Mini Project</a></p>
        </div>
        """
        return footer
